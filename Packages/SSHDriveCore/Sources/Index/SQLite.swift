...
/// database in the project and its access patterns are fixed (docs/design/item-index.md).
public final class SQLiteConnection {
    public enum OpenMode {
        /// The agent's connection. Creates the file, WAL, one writer.
        case readWrite
        /// The extension's connection. A read-only WAL connection still has to open the
        /// -shm file for writing, because readers publish their read marks through it; the
        /// group container is writable by the sandboxed extension, which is what makes a
        /// read-only reader there possible at all.
        case readOnly
    }
    ...
    /// (docs/design/item-index.md), which copies a backup *into* this connection with
    ...
    /// Set by a test only, and nil in the shipping agent: called with every statement's SQL
    /// and the transaction depth it was issued at. It is the only way to assert "a
    /// directory listing is written in one transaction", and that the transaction wraps the
    /// **writes** and nothing else, from outside SQLite (`E2` in `docs/design/testing.md`).
    /// One optional test per statement, off every path that matters.
    ///
    ...
    /// Set by a test only: called only when `prepare` actually runs `sqlite3_prepare_v2`,
    /// so a test can assert that the number of *compilations* a listing or an `item(for:)`
    /// storm costs is a constant and not a multiple of the number of rows.
    ...
    ///
    /// The index runs a fixed and very small set of statements over and over: a
    /// 10,000-entry listing is the same three statements ten thousand times, and an
    /// `item(for:)` storm is two. Compiling one per execution is most of what both cost -
    /// 8,006 compilations for a 2,000-entry listing, five per `item(for:)` - so a statement
    /// is reset, cleared and kept instead.
    ///
    ...
    /// Finalises every cached statement. The one caller is the restore, which replaces the
    /// whole database under this connection: `sqlite3_prepare_v2` statements re-prepare
    /// themselves after a schema change, but an idle statement the backup API might call
    /// "in use" is not worth the argument.
    ...
    /// step per anchor - 2,000 of each in a 2,000-entry listing.
    ...
    /// `SQLITE_SCHEMA`, so the `ALTER TABLE`s of `IndexWriter.migrate` and the restore need
    ...
    /// wraps itself in a transaction of its own, so "a directory listing is written in one
    /// transaction" puts a `batch` around a loop that calls `appendAnchor` and `delete`.
    /// Without the depth count the inner `BEGIN IMMEDIATE` fails with "cannot start a
    /// transaction within a transaction" and takes the whole listing with it.
    ...
    /// failure that propagates still rolls the whole thing back. Every multi-row change is
    /// one of these, and several of them legitimately contain others.
    ...
    /// anchor, each compiled once and bound ten thousand times - is prepared once and
    /// executed many times, and `E2` counts executions. So each reuse says so.
    ...
    /// transaction buys nothing: the statement is atomic by itself, and a throw from inside
    /// `batch` rolls the whole listing back either way.
...
/// for the next caller that asks for the same SQL. The connection is held strongly, so a
/// statement can never outlive the database it was compiled against; the cache holds bare
/// handles, so that costs no reference cycle.
