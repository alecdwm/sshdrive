...
/// lookup of `ssh` when that is not an executable path (docs/design/ssh.md).
...
/// they do for `ssh` (docs/design/locations.md).
public struct SSHTarget: Sendable, Equatable {
    ...
    /// False only for an `agentDependent` location (docs/design/secrets.md).
...
/// Assembles every `ssh` command line the agent runs (docs/design/ssh.md).
///
...
public enum SSHCommandBuilder {
    ...
    /// `sshdrive show` reports them separately (docs/design/ssh.md,
    ...
    public enum Overrides {
        ...
        /// An `ask` here raises a question nobody is there to answer
    ...
    ///
    /// `hostKeyChecking` is `yes` everywhere except the collect connection
    /// (docs/design/secrets.md), which runs `ssh`'s default `ask` (or `accept-new` under
    /// `--trust-first`) so the fingerprint question can be relayed to the terminal and
    /// written to the user's own `known_hosts` exactly as it would have been from a tty.
    ///
    /// `logLevel` is `ERROR` everywhere except the collect connection, which raises it to
    /// `DEBUG1` for that one connection so `ssh` prints the server's identification
    /// string - "remote software version …" - which is the only place the agent can ever
    /// read it: a mux client speaks to the master's socket rather than to the server
    /// (docs/design/ssh.md, docs/design/cli.md). The debug lines are stripped again
    /// before anything looks at that connection's stderr.
    ...
    /// supervise, stderr to read and an exit to watch (docs/design/ssh.md).
    ///
    ...
    public static func master(
        ...
        if let proxyCommand {
            ...
            // bastion does not exist. Measured on macOS 26.4, 2026-09-04.
        ...
        // `sshOptions` too (docs/design/ssh.md): it reaches `ssh -G`, where the chain
        // builder picks it up like one from the config, and it is dropped here. `ssh`
        // takes the first value it sees, so the `ProxyJump=none` above would already
        // have won, but leaving it on the line would make `sshdrive show` read as though
        // we passed it.
    ...
    /// do what the user meant (docs/design/cli.md).
    ...
    /// exchanged (docs/design/ssh.md). The host argument is a placeholder; the mux
    /// protocol uses nothing from it.
    ...
    /// from the user, the config or the server ever appears on it
    ...
    /// whether it is alive; it says nothing about the server (docs/design/ssh.md).
    ...
    /// (docs/design/locations.md). `ignoreConfigFiles` is the `-F /dev/null` half of the
    /// attribution diff: a value that differs between the two came from a config file,
    /// and `-F` silences `/etc/ssh/ssh_config` as well as the user's file.
