...
public enum SSHProcessError: Error, LocalizedError {
    ...
    /// The key agent's socket is missing or refusing; `ssh` was never run
    /// (docs/design/ssh.md).
    ...
    /// first bytes received so the user can find the rc file (docs/design/security.md).
    ...
    /// What the agent does about it, where that is defined (docs/design/ssh.md).
