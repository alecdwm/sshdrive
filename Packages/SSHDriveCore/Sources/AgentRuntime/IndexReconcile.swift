...
enum IndexReconcile {
    ...
    static func reconcileAgainstReplica(
        ...
        guard let mount = await mountURL(locationID: locationID, replica: replica) else {
            ...
            for entry in entries {
                ...
                guard
                    let identifier = await replicaIdentifier(
                        at: entry, locationID: locationID, expiry: expiry, replica: replica)
                else {
                    report.errors.append(
    ...
    private static func mountURL(
        locationID: String, replica: any ReplicaControlling
    ) async -> URL? {
        try? await Deadline.run("asking for the domain's user-visible URL") {
            try await replica.userVisibleURL(
                locationID: locationID,
                identifier: ProviderItemIdentifier.rootContainer.rawValue)
        }
    }
    ...
    private static func replicaIdentifier(
        ...
        // A path that answers for another domain is not ours to write a row for. The
        // adapter applies that filter, because it is the only side that sees the domain
        // the system named; it should not be possible under our own mount anyway.
        let outcome: String?? = try? await Deadline.run(
            "asking the system for an identifier", seconds: remaining
        ) {
            await replica.identifierForUserVisibleFile(at: url, locationID: locationID)
        }
        return outcome ?? nil
    }
    ...
    private static func attributes(
        ...
        return SFTPFileAttributes(
            ...
            mtime: Int64(StatTimes.modificationSeconds(of: buffer)),
            mode: mode,
            uid: identity.uid ?? 0,
            gid: identity.gid ?? 0,
