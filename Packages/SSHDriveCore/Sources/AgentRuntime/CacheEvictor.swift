...
public actor CacheEvictor {
    ...
    public private(set) var lastPass: [String: Any] = [:]
    ...
    public init(
        locationID: String, runtime: LocationRuntime, ttl: CacheTTL,
    ...
    public func start(now: Double = Date().timeIntervalSince1970) {
    ...
    public func setTTL(_ value: CacheTTL) {
        ttl = value
        Log.agent.notice(
    ...
    public func nextRunAt() -> Double { lastRun + CacheEvictor.interval }

    public func setTTLOverride(seconds: Double?) {
    ...
    public func runPass(
        ...
        let started = Date()
        guard let identifiers = await environment.replica.materializedIdentifiers(
        ...
        for decision in wanted {
            let report = await environment.replica.evict(
    ...
    public func evictPath(_ pathString: String) async throws -> [String: Any] {
        ...
        var report = await environment.replica.evictWithRetry(
            locationID: locationID, identifier: identifier, attempts: 5, subject: pathString,
            sleeper: environment.clock)
        if report["evicted"] as? Bool == true {
    ...
    public func evictAll(unpinAll: Bool) async throws -> [String: Any] {
        ...
        if markers.pinRoots.isEmpty && pinsRemoved == 0 {
            let root = await environment.replica.evictWithRetry(
                locationID: locationID, identifier: IndexWriter.rootIdentifier,
                attempts: 3, subject: "the location root", sleeper: environment.clock)
            if root["evicted"] as? Bool == true {
                report["mode"] = "root container"
                report.merge(root) { new, _ in new }
                let identifiers =
                    await environment.replica.materializedIdentifiers(locationID: locationID) ?? []
        ...
        for candidate in candidates {
            ...
            let outcome = await environment.replica.evictWithRetry(
        ...
        -> [EvictionPlan.Candidate]
    {
        ...
        for var candidate in candidates {
            ...
            if let url = try? await environment.replica.userVisibleURL(
                locationID: locationID, identifier: candidate.identifier),
                let times = environment.replica.replicaTimes(url: url)
            {
