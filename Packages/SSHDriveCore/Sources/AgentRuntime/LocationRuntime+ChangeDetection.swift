...
extension LocationRuntime {
    ...
    public struct ChangeApplication: Sendable {
        public var changed = 0
        public var deleted = 0
        public var held = 0
        public var released = 0
        public var listedDirectories = 0
        public var errors: [String] = []

        public init() {}

        public var isEmpty: Bool {
            changed == 0 && deleted == 0 && held == 0 && released == 0
        }

        public mutating func absorb(_ result: LocationRuntime.ReconcileResult) {
    ...
    public func currentRootSet() throws -> RootSet {
    ...
    public func listOne(_ pathBytes: Data) async -> ReconcileResult {
        do {
    ...
    public func applySweepHits(_ hits: [SweepHit], now: Double = Date().timeIntervalSince1970) async
        -> ChangeApplication
    {
        var application = ChangeApplication()
        var dirty: Set<Data> = []
    ...
    public func applyHelperEvents(_ events: [HelperEvent], now: Double = Date().timeIntervalSince1970)
    ...
    /// `x`.
    public static func stripLeadingDot(_ path: Data) -> Data {
        if path == Data(".".utf8) { return Data() }
        if path.starts(with: Data("./".utf8)) { return path.dropFirst(2) }
        return path
    }

    public func writerInFlightPaths() async -> Set<Data> { await writer.inFlightPaths() }
    ...
    public func setSweepServerTime(_ value: Int64, localNow: Double = Date().timeIntervalSince1970) {
    ...
    public func paths(forIdentifiers identifiers: [String]) -> Set<Data> {
        var out: Set<Data> = []
    ...
    public func recordWatchCycle(_ outcome: [String: Any]) { lastWatchCycle = outcome }

    /// What `status` prints for change detection when the detector itself is not to hand.
    public func watchReport() -> [String: Any] {
        var out: [String: Any] = [:]
