...
/// (`MirroredProviderConstantsTests`), for the same reason: a number that drifts silently
/// is the only kind of breakage a Linux suite cannot see.
///
/// Nothing above the seam **branches** on the eviction codes - the code says nothing about
/// why (`MQ-017`, `MQ-018`, `MQ-019`) - but they travel in the reports `status` and the
/// runbooks read, and the quirk table names them as numbers, so they are asserted here
/// rather than left as prose.
public enum MirroredAgentConstants {

    // MARK: The replica's `stat`
    ...
    // MARK: The domain's testing modes
