...
public actor LocationRuntime {
    ...
    public var keptAndMaterialized: Set<String> = []
    ...
    private var isRemoteBacked: Bool {
        transport is (any LiveConnection) || transport is ReconnectingTransport
    }
    ...
    public func shutdownTransport() async {
        if let reconnecting = transport as? ReconnectingTransport {
            await reconnecting.gate.shutdown()
            return
        }
        if let ssh = transport as? any LiveConnection { await ssh.shutdown() }
    }
    ...
    public func snapshot(identifier: String) throws -> SSHDriveItemSnapshot {
    ...
    public func cancel(transferID: String) async {
        await scheduler.cancel(transferID: transferID)
    }

    public func schedulerStatistics() async -> TransferScheduler.Statistics {
    ...
    public func modifyItem(
        ...
        do {
            ...
            if changedFields.contains(.fileSystemFlags), let newFileSystemFlags,
                row.type != "symlink"
            {
                let flags = ProviderFileSystemFlags(
    ...
    public func setCatchUpSweep(enabled: Bool) {
    ...
    public func dumpAnchors(limit: Int) throws -> [IndexAnchorEntry] { try index.anchors(limit: limit) }
    public func dumpRoots() throws -> [(path: Data, reason: String, lastSeen: Double)] { try index.roots() }

    public func backupIndex() throws {
    ...
    public func dumpFakeTree() async throws -> [(path: String, type: String, size: Int64, mode: UInt32)] {
        guard let fake = transport as? FakeTransport else {
            throw SSHDriveAgentError.notImplemented.asNSError(
                "This location does not run on the fake backend.")
        }
    ...
    public func seedFakeTree(fileCount: Int) async throws {
