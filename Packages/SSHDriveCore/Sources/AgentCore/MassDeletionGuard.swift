...
/// The mass-deletion guard (docs/design/change-detection.md).
///
...
/// `realpath` check (docs/design/security.md) is satisfied because the mount point itself
/// is still there. Reporting that literally would delete every item beneath it from the
/// replica: the cache, every local xattr and Finder tag with it, and every item would come
/// back under a new identifier, since there are no tombstones (docs/design/item-index.md).
///
...
public struct MassDeletionGuard: Sendable {
    ...
    public struct Decision: Sendable, Equatable {
        ...
        /// The `sshdrive status` line, e.g. "14 deletions held in Photos". Nil when
        /// nothing is held.
    ...
    public struct Input: Sendable {
        ...
        /// directory of dotfiles is not what "half of a directory" is counting.
    ...
    /// non-hidden items and at least 20 of them, or would empty a root that held anything
    /// at all, the missing items are not reported.
    ///
    /// The pending half is independent of every count. Reporting an item with a pending
    /// local edit deleted through the working set does not
    ...
    /// collided create for ever with no alert (docs/design/writes.md). The user's edit
    /// then sits
    ...
    /// Every path in `paths`, plus every ancestor directory of each. Byte-level: server names
    /// need not be valid UTF-8 (docs/design/names-and-attributes.md). The location root - the
    /// empty path - is never added; a listing cannot report the root missing.
    ...
    /// The `sshdrive status` line: "14 deletions held in Photos".
