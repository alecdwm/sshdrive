...
/// dialect, over which window, and the POSIX `sh` body that runs it
/// (docs/design/change-detection.md, docs/design/security.md).
///
...
/// never reaches a shell as text (docs/design/security.md).
public struct SweepPlan: Sendable, Equatable {

    /// One `find` invocation's worth of roots. Two invocations, because `-maxdepth`
    /// applies to every starting point of one `find`, and each is run in batches of at most
    /// 64 KB of root arguments, since the roots reach `find` as its argv and a few thousand
    ...
    /// 64 KB of root arguments per invocation.
    public static let argumentByteBudget = 64 * 1024

    /// The GNU format, byte for byte. With it every hit arrives with its type, size,
    /// nanosecond mtime, inode, mode and owner and needs no follow-up `stat`.
    ...
    /// file under the roots (docs/design/change-detection.md).
    ...
    public init(shallowRoots: [String], recursiveRoots: [String], excluded: [String] = [],
        ...
        // The probe is the authority, but busybox is checked again here: busybox has no
        // `-cmin` at all, and BusyBox v1.36.1 as shipped by current Alpine answers
        // `find: unrecognized: -cmin`. A probe that got that wrong would not lose one
        // field, it would make every sweep on that server exit non-zero with nothing on
        // stdout.
        self.usesCmin = takesCmin && flavour != .busybox
        self.takesPrintf = takesPrintf
    }

    /// True when the sweep asks for the `-printf` record rather than `-print0`, so every hit
    /// arrives complete and needs no follow-up `stat` over SFTP
    /// (docs/design/change-detection.md, docs/design/item-index.md). Only GNU `find` has
    /// `-printf`; BSD and busybox get bare paths.
    ...
    /// How one cycle's roots divide between the two tiers (docs/design/change-detection.md).
    ///
    ...
    ///   `set --` is a String pipeline end to end (docs/design/security.md):
    ///   `RemoteScript` builds one `set --` line out of `[String]`, so such a root has no
    ///   spelling that survives the trip. It is left out of the argv and listed at tier 0
    ///   in the same cycle instead.
    ///
    ...
    /// `-path` takes a glob, so `*`, `?`, `[` and `\` in an excluded path are
    /// backslash-escaped before the pattern is embedded: `-path 't/[x]'` does not match a
    /// directory named `[x]` (verified on GNU `find`), and an exclusion that silently
    ...
    ///
    /// Nothing from the user is in the body: a directory on a shared NAS named
    /// `$(rm -rf ~)` must never reach that shell, so every root and every excluded
    ...
    ///
    /// The server's own clock is printed first, before any `find` runs. The window `N` is
    /// computed from the **server's** clock, never the Mac's: measured on the Mac's clock,
    /// a server running a few minutes behind would silently miss every change until the
    ...
    /// The `find` expression appended after the roots, in the order
    /// docs/design/change-detection.md states it: the depth limit, then the prunes, then
    /// the type test, then the time test, then the output.
    ///
    ...
    ///
    /// Both files and directories are matched: a directory's ctime changes on create,
    /// delete and rename inside it, but an in-place edit changes only the file's own, so
    /// the file test is needed too. There is no `-xdev`: a NAS root routinely contains
    /// separate mounts, and containment comes from not following links
    /// (docs/design/security.md), not from staying on one filesystem.
    private func expression(recursive: Bool) -> String {
        ...
        if let windowMinutes {
            // `-cmin` (change time) rather than `-mmin`: ctime moves whenever mtime does, and
            // also on chmod, chown and on writes that preserve mtime (rsync -t, cp -p, touch
            // -r), all of which change our content or metadata version
            // (docs/design/item-index.md). busybox has none, so it takes -mmin and loses
            // those cases, which `status` reports as a note. An integer of our own is the
            // only literal in this string.
