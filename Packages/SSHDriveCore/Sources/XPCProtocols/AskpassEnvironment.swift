...
/// The three variables the agent puts in every `ssh` it spawns (docs/design/secrets.md):
///
...
/// `DISPLAY`; without it OpenSSH only reaches for an askpass when it has no terminal *and*
/// `DISPLAY` is set. A `ProxyJump` hop inherits all three from the master, which is how it
/// gets a token the agent never issued it directly (docs/design/secrets.md,
/// docs/design/ssh.md).
///
...
/// reads the token out of its own environment.
public enum AskpassEnvironment {
    ...
    /// each use their own (docs/design/secrets.md).
    ...
    /// classifier would have to read as an authentication failure (docs/design/ssh.md).
...
/// tests drive through `AskpassHarness` (docs/design/secrets.md).
