...
/// SSH Drive has no trash (docs/design/names-and-attributes.md): `allowsTrashing` is never
/// set, a delete is a delete, and nothing invents a server-side trash that other SFTP clients
...
public enum SSHDriveTrash {
    ...
    /// (docs/design/names-and-attributes.md), so a name that would land on the system's own
    /// trash is refused however it is spelled.
