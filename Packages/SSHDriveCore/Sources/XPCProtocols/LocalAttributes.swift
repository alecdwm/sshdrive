...
///
/// Both are stored in one blob, the row's `xattrs` column, because
/// docs/design/item-index.md hashes exactly that blob into the metadata version. Tags are
/// not an xattr: `com.apple.metadata:_kMDItemUserTags` is excluded from
/// `extendedAttributes` deliberately and arrives as the item's own `tagData`, so they get
/// their own field rather than a reserved key inside the dictionary, where a real xattr of
/// that name would collide with them.
///
...
public struct LocalAttributes: Codable, Equatable, Sendable {
    ...
    /// column existed.
    ///
    /// **`.sortedKeys` is load-bearing.** This blob is hashed into the metadata version
    /// (docs/design/item-index.md), and `JSONEncoder` does not promise a key order without
    /// it - not for the `xattrs` dictionary and not for the struct's own two keys. Without
    /// it the same attributes encode to two different byte strings within one process, the
    /// hash moves, and the system re-reads every item it holds for no reason.
    /// `RowBuilderTests.testTheSameAttributesTwiceProduceTheSameVersion` is what catches
    /// a regression, intermittently.
