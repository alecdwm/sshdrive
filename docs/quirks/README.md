...
does ([macos.md](macos.md)) and what a remote server does ([servers.md](servers.md)). It exists so that a future macOS
or a new kind of server is a **new column in a table**, not a new test; [docs/design/testing.md](../design/testing.md)
...
name the ids they defend (`grep -rn 'MQ-0' Packages/SSHDriveCore/Tests`). Two tests in the suite
check one direction each. `QuirkCatalogueTests` (`SystemModelTests`) fails when an `MQ` id that
`SystemModel`'s quirk table reads is not a row of [macos.md](macos.md), and when a modelled id has no
measurement covering a supported version. `ServerProfileScenarios` (`ServerModelTests`) fails when
an `SQ` id that `ServerModel` keys a rule on is not a row of [servers.md](servers.md), when a profile cites an id
no rule implements, and when a handful of measured values in the profiles differ from the rows.
