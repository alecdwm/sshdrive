...
(`docs/design/names-and-attributes.md`), because they break a collision between
...
`readlink` (`docs/design/symlinks.md`); those go out sixteen in flight rather
...
set, `docs/design/cli.md`) rather than one number.
The client exposes a `protocol SFTPTransport` whose methods take
`RelativePath` values only (`docs/design/security.md`).
...
transfers, the eager downloads of a kept subtree (`docs/design/pinning.md`)
...
`MaxSessions` of 2 (`docs/design/ssh.md`) the same scheduler runs on the
...
(`docs/design/ssh.md`). And the status reply carries no errno. OpenSSH's
...
- The libraries on offer each fall short somewhere: `swift-nio-ssh` has no
  ...
  (`docs/design/writes.md`), has no `limits@openssh.com`, needs its own
