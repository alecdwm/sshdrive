...
`sshdrive add nas` printed the helper upload notice and then **"4/8 optimal"** with
...
nas` said **"6/8 optimal"**, `helper 0.1.0 at /home/alec/.cache/sshdrive (push, ~1s)`
...
evidence here.** The owner's report showed both as absent on the Tailscale SSH server and present on
the OpenSSH one, same client. Audited end to end: `SFTPClient.dispatch` parses the whole
...
notice level, so the next report from that server answers the question outright:
...
3. **`/home/alec/.cache/sshdrive is writable by others` on the OpenSSH server: fixed.** `mkdir`'s
...
5. **The helper dying 15 s after `ready`: not reproduced, instrumented instead.** The
   ...
   `sshdrive logs nas` from the owner after this build; a testbed reproduction needs a
