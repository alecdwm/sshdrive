...
**2026-09-08, step 8 of the testing architecture: `AgentRuntime`.** Everything `Apps/Agent`
decided moved into the package behind the fifteen protocols of
`docs/testing-architecture.md` section 2.3, so the agent's logic now runs on Linux with no
Mac, no network and no keychain. `Apps/Agent` is ten adapter files and 2,009 lines, down
from 13,300, and the rule that has applied to `Apps/FileProvider` since step 1.3 applies to
it too: nothing there may contain a branch worth testing. The seams are one
`AgentEnvironment` that `DomainManager` is constructed with rather than a set of
singletons, which is what lets two agents exist in one test process; the XPC command layer,
which is reached from an object made per connection, finds its agent through
`AgentCommandContext`, a task local. Beside `AgentRuntime` is
`AgentRuntimeTestSupport` - a fake for every seam, a driven clock, and `AgentHarness` - and
sixteen scenarios in `Tests/AgentRuntimeTests`: **F6-F10, D5, G1, G4, G5, K13, K14, P1, P2**
and **P8**.
...
| `Packages/.../AgentRuntime` | **Everything the agent decides** (moved out of `Apps/Agent` on 2026-09-08, step 8 of `docs/testing-architecture.md`), so it compiles and is tested on Linux: `LocationRuntime` with `+ChangeDetection` and `+Pinning` (the listing reconcile, the fetch through the peer's `FileHandle`, create/modify/delete, the catch-up sweep, the guard-aware listing diff, the `held` table, `accept-deletions`, the root-set refresh, the tier 0 cycle, the sweep's application to the index, and section 7.1.1's marker algebra), `DomainManager` (the domain policy, the per-location runtimes, gates, detectors and evictors, `ReconnectSequence`'s order on every reconnect, sleep/wake/path/unlock, `shutdownAll` and the stranded-domain sweep), `ChangeDetector`, `CacheEvictor`, `IndexReconcile`, `ReconnectingTransport` with `ConnectionGate`, `SSHBackedTransport` and `SSHTransportLauncher`, `ChannelBudget`/`ChannelProbe`/`CapabilityCache`, `HelperDeployer`/`HelperStream`/`HelperCleanup`, `CollectConnection`, `LocationCommands`, `ControlCommands`, `TransportDebug`, `ConfigAccess`, `Deadline`, `CallJournal`, `HandleSink`, `AgentSecrets`, and three types that used to be a decision inline in an adapter: `AgentLifecycle` (what counts as a replacement bundle, and the unregister role's wait for launchd), `PresenceOverride` and `ForcedNotReady`. Behind `ReplicaControlling`, `LoginItemControlling`, `LaunchdControlling`, `PowerObserving`, `NetworkPathObserving`, `PresenceReporting`, `ScreenLockObserving`, `PeerIdentifying`, `IndexReaderPeering`, `BundleInspecting`, `KeychainDiagnosing`, `TransportLauncher`, `TerminalRelaying`, `AgentEndpoint` and `AgentClock` - one `AgentEnvironment`, handed to `DomainManager` at construction, which is what lets two agents exist in one test process. |
| `Packages/.../AgentRuntimeTestSupport` | An in-memory implementation of every seam above, and `AgentHarness`, which is one agent with a group container of its own: `FakeReplica` (records every domain, signal, enumerator, eviction and lookup call, and stages a refusal, a missing manager, or the 4099 that arrives after `add(domain)` has landed), `FakeLoginItem` and `FakeLaunchd` (`MQ-062` and `MQ-063` as state machines - a register that does not repair, and the window `unregister()` returns inside), `ScriptedPower`, `ScriptedNetwork`, `ScriptedPresence`, `ScriptedScreenLock`, `ScriptedPeers`, `RecordingReaderPeers`, `FakeBundle`, `RecordingEndpoint`, `FakeTransportLauncher` with `FakeLiveConnection` (over `SFTP.FakeTransport`, or over `ServerModel`'s channels through its `transportFactory` hook), and `VirtualAgentClock` (wall clock and monotonic together; a sleep parks until the test advances the clock, so the eviction backoff, the breaker's schedule and the ladder's climb-back run at their real numbers and cost nothing). |
| `Apps/Agent` | **Ten adapter files, 2,009 lines, and no decision** (2026-09-08): `main.swift` (the three roles - launchd, `unregister`, and the `open -g` registrar - and nothing else), `ListenerDelegate` (the `NSXPCListener` on the group-prefixed mach service with `setCodeSigningRequirement`, and which interface each peer is handed), `AgentService` (the object exported per connection, forwarding every XPC method), `AskpassService` (the one-method askpass interface), `CLIRelay` (`TerminalRelaying` over the CLI's callback object), `ExtensionPeers` (`IndexReaderPeering` over the live `NSXPCConnection`s), `PeerExecutable` (`proc_pidpath`), `AgentLifecycle` (the SIGTERM source and the `O_EVTONLY` vnode watch; what they decide is in the package), `FileProviderReplica` (`ReplicaControlling` over `NSFileProviderManager` - the domain list, the signals, both replica enumerators, `evictItem`, both identifier lookups, the `lstat`, and the stabilization and testing-mode hooks the spikes need) and `SystemSeams` (IOKit power with its `IOAllowPowerChange`, `NWPathMonitor`, `CGEventSource`/`CGSessionCopyCurrentDictionary` presence, the two screen-lock distributed notifications, `SMAppService`, `launchctl print`, the bundle and its quarantine xattr and `pluginkit` line, the keychain diagnostics, and `AgentEnvironment.runningOnMacOS`, which is every one of them in one value). |
...
The File Provider half of these is `Apps/Agent/FileProviderReplica.swift` (it was
`SpikeHooks.swift` until step 8 folded it into `ReplicaControlling` on 2026-09-08); the
fault injection and the transfer accounting are on `LocationRuntime`. They are in the agent
...
share one implementation (`ReplicaControlling`, `FileProviderReplica` on Darwin):
**`evict <name> <path>`** is the same
