...
107. **LaunchServices keys its records on the bundle identifier, and a record for a second copy of the app answers every registration of the installed one.** Opening the app once from the mounted DMG leaves a record for `/Volumes/SSH Drive/SSH Drive.app`, and detaching the volume does not remove it. While it is there, `lsd` answers every registration of `/Applications/SSH Drive.app` - the cask's own `lsregister -f -R -trusted` included - with `SecStaticCodeCreateWithPath(<private>) failed with error -67028`, `skipping registration of an incomplete bundle` and `Registration succeeded, but did not actually register anything new; returning existing bundle`, so `pluginkit -m` prints nothing, `doctor` fails "extension registered" and "file provider domains", and no domain can be added, on a bundle with no quarantine attribute at all. `lsregister -u` on the stale path and then the force registers the appex at once. The cask's postflight and the app launch both sweep before they force, and `doctor` lists every record path that is not the installed bundle (measured on macOS 27.0, 2026-09-23, upgrading 0.1.7 to 0.1.8 with a three-hour-old DMG record; 26.4.1 produces no `/Volumes` record from the same upgrade). **And straight after Homebrew moves the bundle, LaunchServices answers nothing at all:** on the 0.1.8 to 0.1.9 upgrade the postflight's `spctl --assess` printed `internal error in Code Signing subsystem`, its `lsregister` printed `failed to scan /Applications/SSH Drive.app: -10822 from spotlight` and never registered anything, and its `open -g` printed `-10810 kLSUnknownErr` with `Couldn't communicate with a helper application` under it, so the app-launch role never ran either; the same commands by hand minutes later worked. The sweep and the force are therefore retried - by the postflight on 5 s sleeps, and by `ensureExtensionRegistered` at 15/60/300 s - and the check runs from the **launchd** role too, which is the one path that always runs, since launchd starts the agent for the mach service whatever else failed (`packaging.md`).
108. **The cask's postflight is a legacy `postflight do` block, because `postflight_steps` runs sandboxed.** Homebrew's cask step sandbox (Homebrew/brew#23907) denies LaunchServices and the Mach services behind it, so inside `postflight_steps` `spctl` answers `internal error in Code Signing subsystem`, `lsregister` answers `-10822` and then dies `Trace/BPT trap: 5`, `open -g` answers `-10810 kLSUnknownErr` / `Couldn't communicate with a helper application`, and nothing registers; the same commands work by hand. That is the sandbox, not a LaunchServices that needs time after the bundle moved, and retrying does nothing. Homebrew keeps the legacy block for third-party taps only, so the caveats carry the manual fallback, `open -a "SSH Drive"` (`packaging.md`).
