...
107. **A LaunchServices record built while the bundle was still being copied hides the appex from PlugInKit, and `open -g` does not rebuild it.** Something registers the bundle mid-copy during a `brew upgrade`, `lsd` logs `no satisfactory executable could be found` and `skipping registration of an incomplete bundle`, and every launch after that is answered `Registration succeeded, but did not actually register anything new; returning existing bundle` - so `pluginkit -m` prints nothing, `doctor` fails "extension registered" and "file provider domains", and no domain can be added, on a bundle with no quarantine attribute at all. `lsregister -f -R -trusted` on the bundle rebuilds the record and `pkd` creates the plugin within milliseconds; unlike `pluginkit -a`, it survives the next launch. The cask's postflight runs it after the quarantine strip, and the app launch runs it once when PlugInKit answers nothing, waiting up to 5 s for the appex to appear (measured on macOS 27.0, 2026-09-23, upgrading 0.1.5 to 0.1.7; 26.4 registers it through the plain `open -g`) (`packaging.md`).
